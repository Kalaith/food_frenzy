...
export const useDishHandling = (showMessage: (message: string) => void) => {
  const { customers, updateCustomer, addScore, canProcessCustomer } =
    useGameStore();
  ...
  const handleDishDropOnCustomer = useCallback(
    (customerId: number, dishColor: string, dishName: string) => {
      ...
      if (isPreferred) {
        ...
        deliciousnessGain = 1;
        showMessage(
          `${customer.type.name} loves ${dishName}! +${satisfactionGain} satisfaction!`,
        );
      } else {
        showMessage(
          `${customer.type.name} ate ${dishName}. +${satisfactionGain} satisfaction.`,
        );
      }
      ...
      const newDeliciousness = Math.min(
        5,
        customer.deliciousness + deliciousnessGain,
      );
